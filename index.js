...
const FileStudent = require('./model/fileStudent');
...
//app.post('/student/login',passport.authenticate('local',{successRedirect:'/pgTeacher',failureRedirect:'/student'}))
...
app.post('/create/excel',upload.single('fil'),(req,res)=>{
	var x = 'uploads/' + req.file.originalname;
	var filename = req.file.originalname;
	//code xử lý
	
	var temp = new FileStudent({
			ClassID: req.session.ClassID,
			TeacherID: req.session.TeacherID,
			//Title: title,
			filepath:x
    ...
    temp.save((err,data)=>{
		if (err) {
			...
			data.forEach(function hash(element) {
				bcrypt.hash(String(element["Pass"]), 10, function (err, hashPassword) {
					if (err) {
						console.log(err);
						next();
					}
...
//passport
//để đây k có xài,đừng xóa của t 
